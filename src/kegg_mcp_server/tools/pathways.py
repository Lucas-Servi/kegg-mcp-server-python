...
def _build(p: dict) -> PathwayInfo:
    return PathwayInfo(
        ...
        description=p.get("description"), organism=p.get("organism"),
        gene=p.get("gene"), compound=p.get("compound"), reaction=p.get("reaction"),
        module=p.get("module"), disease=p.get("disease"), drug=p.get("drug"),
...
def register(mcp: FastMCP) -> None:
    ...
    @mcp.tool()
    async def search_pathways(query: str, organism_code: str = "map", max_results: int = 50, ctx: Context = None) -> SearchResult:
        ...
        return SearchResult(query=query, database=db, total_found=len(results), returned_count=len(limited), results=limited)
    ...
    async def get_pathway_info(pathway_id: str, ctx: Context = None) -> PathwayInfo:
        ...
        Args:
            pathway_id: KEGG pathway ID (e.g. 'hsa00010' for human glycolysis, 'map00010' for reference).
        """
        kegg = ctx.request_context.lifespan_context.kegg
    ...
    async def get_pathway_compounds(pathway_id: str, ctx: Context = None) -> PathwayLinks:
        ...
        return PathwayLinks(pathway_id=pathway_id, linked_db="compound", pairs=pairs, count=len(pairs))
    ...
    async def get_pathway_reactions(pathway_id: str, ctx: Context = None) -> PathwayLinks:
        ...
        return PathwayLinks(pathway_id=pathway_id, linked_db="reaction", pairs=pairs, count=len(pairs))
